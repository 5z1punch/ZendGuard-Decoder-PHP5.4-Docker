...
$_CURRENT_FILE = NULL;
$GLOBALS['output_echo_results'] = "";
function save2buf(){
    $args = func_get_args();
    foreach($args as $value){
        $GLOBALS['output_echo_results'] .= $value;
    }
}
        ...
        foreach ($this->offsets as $i => $dim) {
            if ($this->isObject && $i == $last) {
                ...
                if (is_a($dim,'Decompiler_ListBox') && $dim == $this->value) {
                    save2buf("ERROR: Recursive ListBox.\n");
                    print_r($this);
                    assert(0);
                    //exit;
                    continue;
...
class Decompiler {
        ...
        if ($this->test) {
            ...
            for ($i = 0; $opname = xcache_get_opcode($i); $i++) {
                if (!preg_match("/\\bXC_" . $opname . "\\b(?!')/", $content)) {
                    save2buf("not recognized opcode ", $opname, "\n");
                }
            }
        }
        // }}}
    ...
    function detectNamespace($name) // {{{
    {
        ...
        if (strpos($name, '\\') !== FALSE) {
            ...
            if ($ns != $this->namespace) {
                $this->namespace = $ns;
                save2buf('namespace ', $this->namespace, ";\n\n");
            } else return;
        }
            ...
            if (isset($op['php'])) {
                ...
                if ($curticks != $toticks) {
                    ...
                    if (!$curticks) {
                        ...
                        save2buf($EX['indent'], "declare (ticks=$curticks) {\n");
                    }
                }
                if ($needBlankline) {
                    $needBlankline = FALSE;
                ...
                save2buf($indent, str($op['php'], $indent), ";\n");
                $EX['lastBlock'] = 'basic';
            }
        }
        if ($curticks) {
    ...
    function decompileComplexBlock(&$EX, $range) // {{{
    {
        ...
        if (!empty($firstOp['jmpins']) && $opcodes[$firstOp['jmpins'][0]]['opcode'] == XC_JMP
            ...
            save2buf($indent, 'for (', str($initial, $EX), '; ', implode(', ', $conditionCodes), '; ', implode(', ', $nextCodes), ') ', '{', PHP_EOL);
            $this->beginScope($EX);
            $this->recognizeAndDecompileClosedBlocks($EX, $bodyRange);
            $this->endScope($EX);
            save2buf($indent, '}', PHP_EOL);
        ...
        if ($this->isIfCondition($EX, $range)) {
            ...
            do {
                ...
                save2buf($indent, $isElseIf ? 'else if' : 'if', ' (', str($condition, $EX), ') ', '{', PHP_EOL);
                $this->beginScope($EX);
        ...
        if ($firstOp['opcode'] == XC_JMPZ && !empty($firstOp['jmpouts'])
            ...
            save2buf($indent, 'if (', str($condition, $EX), ') ', '{', PHP_EOL);
            $this->beginScope($EX);
            $this->recognizeAndDecompileClosedBlocks($EX, $range);
            $this->endScope($EX);
            save2buf($indent, '}', PHP_EOL);
        ...
        if ($lastOp['opcode'] == XC_JMPNZ && !empty($lastOp['jmpouts'])
            ...
            save2buf($indent, "do {", PHP_EOL);
            $this->beginScope($EX);
            $this->recognizeAndDecompileClosedBlocks($EX, $range);
            $this->endScope($EX);
            save2buf($indent, "} while (", str($this->getOpVal($lastOp['op1'], $EX)), ');', PHP_EOL);
        ...
        if ($firstOp['opcode'] == XC_SWITCH_FREE && isset($T[$firstOp['op1']['var']])) {
            ...
            $this->beginComplexBlock($EX);
            save2buf($indent, 'switch (', str($this->getOpVal($firstOp['op1'], $EX)), ") {", PHP_EOL);
            save2buf($indent, '}', PHP_EOL);
            $this->endComplexBlock($EX);
            return;
        }
        ...
        if (
            ...
            save2buf($indent, 'switch (',$caseCondOp , ") {", PHP_EOL);
            $caseIsOut = FALSE;
        ...
        if (isset($firstJmpOp)
            ...
            save2buf($indent, 'while (', str($this->getOpVal($firstJmpOp['op1'], $EX)), ") {", PHP_EOL);
            save2buf($body);
            save2buf($indent, '}', PHP_EOL);
        ...
        if (isset($firstJmpOp)
            ...
            save2buf($indent, 'foreach (', str($firstJmpOp['fe_src'], $EX), " as $as ) {", PHP_EOL);
            save2buf($body);
            save2buf($indent, '}', PHP_EOL);
    ...
    function &dop_array($op_array, $indent = '') // {{{
    {
        ...
        for ($i = 0; $i <= $last; $i++) {
            ...
            if (!empty($op['jmpouts']) || !empty($op['jmpins'])) {
                save2buf($i, "\t", xcache_get_opcode($op['opcode']), PHP_EOL);
            }
            // */
        }
        unset($op);
        ...
        $this->dasmBasicBlock($EX, $range);
        for ($i = $range[0]; $i <= $range[1]; ++$i) {
            save2buf($i, "\t", $this->dumpop($opcodes[$i], $EX));
        }
        // */
    ...
    function dasmBasicBlock(&$EX, $range) // {{{
    {
        ...
        for ($i = $range[0]; $i <= $range[1]; $i++) {
            ...
            // save2buf($i, ' '; $this->dumpop($op, $EX); //var_dump($op));
...
//          }
                ...
                case XC_DECLARE_INHERITED_CLASS_DELAYED: // {{{
                    ...
                    if (!isset($this->dc['class_table'][$key])) {
                        save2buf('class not found: ', $key, 'existing classes are:', "\n");
                ...
                case XC_RAISE_ABSTRACT_ERROR:
                    ...
                    break;
                case XC_USER_OPCODE:
                    save2buf('// ZEND_USER_OPCODE, impossible to decompile');
                    break;
                case XC_OP_DATA:
                    break;
                default: // {{{
            ...
            if ($notHandled) {
                save2buf("\x1B[31m * TODO ", $opname, "\x1B[0m\n");
            }
            else {
                $this->usedOps[$opc] = TRUE;
            }
    ...
    {
        for ($i = $range[0]; $i <= $range[1]; ++$i) {
            save2buf($EX['indent'], $i, "\t");
            $this->dumpop($EX['opcodes'][$i], $EX);
        }
        save2buf($EX['indent'], "==", PHP_EOL);
    }
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if (isset($op_array['arg_info'])) {
                ...
                if (!empty($ai['class_name'])) {
                    save2buf($this->stripNamespace($ai['class_name']), ' ');
                ...
                save2buf(str($arg[0], $EX));
            }
            if (isset($arg[1])) {
                save2buf(' = ', str($arg[1], $EX));
            }
        }
    }
    ...
    function dfunction($func, $indent = '', $decorations = array(), $nobody = FALSE) // {{{
    {
        ...
        save2buf($isExpression ? '' : $indent);
        if ($decorations) {
            save2buf(implode(' ', $decorations), ' ');
        }
        save2buf('function', $functionName ? ' ' . $functionName : '', '(');
        $this->dargs($EX);
        save2buf(")");
        $this->duses($EX);
        if ($nobody) {
    ...
    function dclass($class, $indent = '',&$ct = NULL) // {{{
    {
        ...
        save2buf($isInterface ? 'interface ' : 'class ', $this->stripNamespace($class['name']));
        if ($class['parent']) {
            save2buf(' extends ', $this->stripNamespace($class['parent']));
        }
        /* TODO */
        if (!empty($class['interfaces'])) {
            save2buf(' implements ');
            save2buf(implode(', ', $class['interfaces']));
        }
        save2buf("\n");
        save2buf($indent, "{");
        // }}}
        ...
        foreach (array(
            ...
            if (!empty($class[$type])) {
                ...
                foreach ($class[$type] as $name => $v) {
                    ...
                    save2buf($newindent);
                    save2buf($prefix, $name, ' = ');
                    save2buf(str(value($v), $newindent));
                    save2buf(";\n");
                }
            }
        }
        ...
        if ($member_variables) {
            ...
            foreach ($member_variables as $name => $dummy) {
                ...
                if ($static) {
                    save2buf($newindent . "static ");
                }

                // shadow of parent's private method/property dima2k fix
                $mangled = FALSE;
                if (!ZEND_ENGINE_2) {
                    save2buf($newindent . 'var ');
                }
                else {
                    if (!isset($info)) {
                        save2buf($newindent . 'public ');
                    }
                    else {
                ...
                if (isset($value)) {
                    save2buf(' = ');
                    save2buf(str(value($value), $newindent));
                }
                save2buf(";\n");
            }
        }
    ...
    function decompileString($string) // {{{
    {
        ...
        if ($this->dc === FALSE) {
            save2buf("error compling string\n");
            return FALSE;
        }
    }
    ...
    function output() // {{{
    {
        ...
        $tmp = $GLOBALS['output_echo_results'];
        $GLOBALS['output_echo_results'] = "";
        return $tmp;
    }
    // }}}
    function outputUnusedOp() // {{{
    {
        for ($i = 0; $opname = xcache_get_opcode($i); $i++) {
            ...
            if (!isset($this->usedOps[$i])) {
                save2buf("not covered opcode ", $opname, PHP_EOL);
            }
        }
    }
    // }}}
